...
class MainAdapter(private val users: MutableList<Message>, var chatPath : String, private val onClick: (Message) -> Unit) : RecyclerView.Adapter<MainAdapter.DataViewHolder>() {
        ...
        fun bind(msg: Message) {
            itemView.apply {
                ...
                if (msg.data.Text != null) {
                    ...
                    val myDir = File(Environment.getExternalStorageDirectory().toString() + "/saveImageForMMMC")
                    if (!myDir.exists()) {
                        myDir.mkdirs()
                    }
            ...
            return object : Target {
                override fun onBitmapLoaded(bitmap: Bitmap, from: LoadedFrom?) {
                    CoroutineScope(Dispatchers.IO).launch {
                        kotlin.runCatching {
                            val file =
                                File(Environment.getExternalStorageDirectory().path + "/saveImageForMMMC/" + path)
...
//            (users[position].from == "l" && users[position].to == chatPath.substring(19)) ||
//            (users[position].to == "l" && users[position].from == chatPath.substring(19))
//        )
